...
data/
  ...
  judge_calibration_set.jsonl        # labeled safe/unsafe pairs — picks the best guard
...
scorecard/
  ...
  judge_selection/                   # WHICH guard to trust: bake-off on the calibration set
...
- **Quality & advice metrics (8)** — `scorecard/metrics.py`: advice accuracy,
  ...
  referral, personalization. Scored by LLM-as-judge in `judge.py`.
- **Guard selection** — `scorecard/judge_selection/`: ranks candidate guard
  models (ShieldGemma, Llama Guard, Qwen3Guard, Granite Guardian) on a labeled
  finance calibration set, so the judge we trust is chosen empirically.
...
`google/gemma-4-E2B`, `google/gemma-4-E2B-it`, `google/shieldgemma-2b`, and —
for the guard bake-off — `meta-llama/Llama-Guard-3-1B`.
...
# 1. pick the best guard model (writes outputs/judge_bakeoff.md)
python -m scorecard.judge_selection.run_bakeoff
#    heavy candidate, run alone:
python -m scorecard.judge_selection.run_bakeoff --only granite-guardian-3.2-5b

# 2. core scorecard: generate responses, judge them, aggregate.
#    Plug the bake-off winner in as the judge; pass 3 judges to compare (info-sheet requirement).
python -m scorecard.run_scorecard \
  --judge-model-ids google/shieldgemma-2b <second_judge> <third_judge>

# 3. cross-lingual safety gap (English + 1 high-resource + 1 low-resource)
...
| `outputs/judge_bakeoff.md` / `.csv` | `judge_selection` | ranked guard models |
| `outputs/xlingual_safety_gap.md` / `.csv` | `cross_lingual` | refusal-rate gap per language |
...
The benchmark reference answers are not perfect ground truth; the LLM judge shares
failure modes with the target model; guard taxonomies don't natively cover
financial crime (hence the custom finance policy in `judge_selection`); and
machine translation adds noise to the cross-lingual test, worst in low-resource
languages. Each submodule's README states its specific caveats.
